...
class SettingsViewModel : ViewModel() {
    ...
    fun changeEmail(email: String, context: Context) {
        viewModelScope.launch {
            showLoading = true
            try {
                val res = BankEnd.changeEmail(Session.userId, email)
                if (res) {
    ...
    fun changePhone(phone: String, context: Context) {
        viewModelScope.launch {
            showLoading = true
            try {
                val res = BankEnd.changePhone(Session.userId, phone)
                if (res) {
    ...
    fun changePassword(oldPassword: String, newPassword: String) {
        viewModelScope.launch {
            showLoading = true
            try {
                val res = BankEnd.changePassword(Session.userId, newPassword, oldPassword)
                if (res) {
                    logout = true
                    showSuccess = true
                } else {
